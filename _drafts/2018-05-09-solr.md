...
Solr的每个集合可以进行配置。例如在`server/solr/核/core.properties`文件中的`collection.configName`属性指定了所用的配置名：
...
- `UUIDField`支持惟一标识符，传入NEW则Solr会新建一个
...
#   separator：字段分隔符，默认","
#   trim：移除两边空格，默认false . This parameter can be
...
- `extractOnly`是否提取内容并返回而不索引，默认false
- `extractFormat`提取内容的序列化格式，可以为默认的xml或text，只用于extractOnly时
- `fmap.源字段`指定把Tika提取到字段对应到哪个文档字段
- `ignoreTikaException`是否在出现异常时仍然索引部分元数据
- `literal.字段`指定字段的一个值
- `literalsOverride`指定覆盖字段的其它值，默认true
...
- 转换器用于处理字段值，可在`entity`标签中属性`transformer`用`,`分隔列表的形式指定需要用到的转换器。
    ...
    - `ScriptTransformer`容许用脚本语言如Javascript、JRuby、Jython、Groovy和BeanShell转换，函数应当以行（类型为Map<String,Object>）为参数，可以修改字段。脚本应当写在数据仓库配置文件顶级的`script`元素内，而转换器属性值为`script:函数名`。
