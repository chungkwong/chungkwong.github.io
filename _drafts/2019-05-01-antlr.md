...
写好语法后我们让ANTLR自动为我们生成解析器代码。在首次使用前需要先下载ANTLR如`wget https://www.antlr.org/download/antlr-4.7.1-complete.jar`。接着就可以运行ANTLR`java -jar antlr-4.7.1-complete.jar -package com.github.chungkwong.json -o src JSON.g4`，其中`-o`选项用于指定输出位置、`-package`选项用于指定生成类所属的包。（版本号可以自行改成最新的）
