var createEquationEditor = function (container) {
	...
	var updateEquation = function () {
		...
		if (!historyRecoving) {
			...
			if (historyIndex >= historyLimit) {
				historyList.splice(0, historyIndex - historyLimit + 1);
	...
	var insertSymbol = function (name) {
		...
		if (/\d/.test(name)) {
			type = 'mn';
		} else if (name.length == 1 && (name.toUpperCase() != name || name.toLowerCase() != name)) {
			type = 'mi'
		} else {
			type = 'mo';
		}
	...
	COMMANDS.set("\\tan", "tan");
	COMMANDS.set("\\cot", "cot");
	COMMANDS.set("\\sinh", "sinh");
	COMMANDS.set("\\cosh", "cosh");
	COMMANDS.set("\\tanh", "tanh");
	COMMANDS.set("\\coth", "coth");
	COMMANDS.set("\\arcsin", "arcsin");
	COMMANDS.set("\\arccos", "arccos");
	COMMANDS.set("\\arctan", "arctan");
	COMMANDS.set("\\arccot", "arccot");
	COMMANDS.set("\\arcsinh", "arcsinh");
	COMMANDS.set("\\arccosh", "arccosh");
	COMMANDS.set("\\arctanh", "arctanh");
	COMMANDS.set("\\arccoth", "arccoth");
